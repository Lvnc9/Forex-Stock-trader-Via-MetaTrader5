...
**Still open (optional):** human Windows demo MT5 smoke (HTF rules + library SL/TP — see [agent/README.md](agent/README.md); do not claim pass without evidence); hedge/multi-position; tick-mode intrabar; walk-forward UI; H&S hand-label calibration on real FX (see [untilNow.md](untilNow.md)). Phases A–E + library SL/TP + automated LiveWorker smoke are in-repo on `main`.
